...
The PAT itself is not used as the credential for API calls. The SDK exchanges it through the Cloud for an access token and a refresh token, and those are what subsequent API calls carry as Bearer tokens.
...
This is the natural choice for interactive scripts, developer tools, and one-off automations where the person running the script can complete the approval step at the time of authentication. Outside of who approves the request, the resulting token pair is identical to one obtained through a PAT.
...
This is the natural choice when the surrounding system already manages tokens for the script. Examples include a parent process that ran the device flow itself and passed the result down, a service that mints tokens out-of-band, or tests that need deterministic credentials.
