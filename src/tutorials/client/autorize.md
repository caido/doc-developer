...
::: info
The [`@caido-community/autorize`](https://www.npmjs.com/package/@caido-community/autorize) package is the spec for the Autorize plugin. The Client SDK uses it to type the calls in this tutorial. See [Use a Plugin's NPM Spec Package](/guides/client/spec_typing.md) for the broader concept.
:::
...
Authorization tests run against existing requests in the project's HTTP history. To get the request ID for a target, use [`client.request.list()`](/guides/client/extract_requests.md) with an [HTTPQL](https://docs.caido.io/reference/httpql.html) filter on the host:
...
::: tip
For scripts that create many templates, subscribe to the `template:completed` event instead of polling. See [Receive Plugin Events](/guides/client/receive_events.md). Polling is shown here because it keeps the example self-contained.
:::
...
2. **Plugin handle**: looks up Autorize by manifest ID, installing it via the SDK if it is not yet present. See [Install a Plugin](/guides/client/install_plugin.md).
...
4. **Target lookup**: queries the HTTP history with an HTTPQL filter to find a request to test. See [Extract Requests](/guides/client/extract_requests.md) and the [HTTPQL reference](https://docs.caido.io/reference/httpql.html).
...
- Subscribe to the `template:completed` event instead of polling, as described in [Receive Plugin Events](/guides/client/receive_events.md).
